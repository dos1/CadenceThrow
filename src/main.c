...
char* GetDataFilePath(char* filename) {
	...
	TestPath("../share/cadencethrow/data/");
	TestPath("../data/");
 ...
 */
	...
	ALLEGRO_BITMAP *icon = al_load_bitmap(GetDataFilePath("icons/cadencethrow.png"));
	al_set_window_title(game.display, "Cadence Throw");
		...
		if (al_is_event_queue_empty(game.event_queue)) {
			...
			if ((ev.type == ALLEGRO_EVENT_TIMER) && (ev.timer.source == game.timer)) {
				...
				if ((ev.type == ALLEGRO_EVENT_KEY_DOWN) && (ev.keyboard.keycode == ALLEGRO_KEY_TILDE)) {
					...
					char filename[255] = { };
					sprintf(filename, "CadenceThrow_%ld_%ld.png", time(NULL), clock());
